...
export const signInWithGoogle = async (): Promise<GoogleAuthResult> => {
  try {
    ...
    if (existingUser) {
      ...
      // Always update avatar if photoURL exists and is different
      if (firebaseUser.photoURL && firebaseUser.photoURL !== existingUser.avatar) {
        updates.avatar = firebaseUser.photoURL;
        needsUpdate = true;
        console.log('📷 Updating avatar from:', existingUser.avatar, 'to:', firebaseUser.photoURL);
      }
      
      // Update name if displayName exists and is different
      ...
      const newUserData = {
        ...
        email: firebaseUser.email,
        avatar: firebaseUser.photoURL || undefined, // Ensure Google profile picture is saved
        joinDate: new Date().toISOString().split('T')[0],
        subscription: 'free' as const,
        authProvider: 'google' as const
      };

      console.log('💾 Creating new user in Firestore with profile picture:', {
        name: newUserData.name,
        email: newUserData.email,
        hasAvatar: !!newUserData.avatar,
      ...
      console.log('👤 Returning new user with avatar:', newUser.avatar ? 'Yes' : 'No');
